...
TerraformによりGitHub Actions用Workload Identity Pool/provider、deployer/runtimeサービスアカウント、Secret Manager secret、必要API、state bucketを作成済みである。GitHub Actions workflowはまだ存在せず、Secret Managerの2つのsecretにはpayload versionをまだ追加していない。明示的なArtifact Registry repositoryは作成していない。
